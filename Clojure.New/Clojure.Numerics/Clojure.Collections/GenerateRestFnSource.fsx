...
let writeDoInvokes(max:int) =
    ...
    let argList(n) =
        let startingParams = 
            ...
            |> Seq.toArray
            |> String.concat ", "
        startingParams + (if n = 0 then "" else ", ") + "args:obj "
...
let writeApplyTo(max:int) =
    ...
    let generateCase(index) =
        let head = sprintf "            | %i -> x.doInvoke( " index
        let optionalFirstArg = if index = 0 then "" else "arglist.first(), "
        let body = if index < 2 then "" else Array.create (index-1) "nl().first()" |> String.concat ", "
        let optComma = if index < 2 then "" else ", "
        let tail = if index = 0 then  "al )" else "al.next() )"
        head + optionalFirstArg + body + optComma + tail
    let lastCase = "            |> _ -> raise <| WrongArityException(-1)"

    fprintfn tw "%s" preamble
    for i = 0 to max do fprintfn tw "%s" (generateCase i)
