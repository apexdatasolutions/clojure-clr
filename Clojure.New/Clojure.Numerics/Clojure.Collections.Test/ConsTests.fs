...
let aseqTests = 
    testList "ASeqTests" [
        ...
        testCase "ASeq ICollection.CopyTo" <| fun _ ->
        ...
        testCase "ASeq ICollection miscellaneous" <| fun _ ->
        ...
        testCase "ASeq.GetEnumerator()" <| fun _ ->
            let ic = makeConsChain 3 :> ICollection  
...
let seqIteratorTests = 
    testList "SeqIteratorTests" [

        testCase "SeqIterator on null" <| fun _ ->
            let e = new SeqEnumerator(null) :> IEnumerator
            Expect.isFalse (e.MoveNext()) "Cannot MoveNext on null"

        testCase "SeqIterator on null has no current" <| fun _ ->
            let e = new SeqEnumerator(null) :> IEnumerator
            let f () = e.Current |> ignore
            Expect.throwsT<InvalidOperationException> f "Current on null should throw"

        testCase "SeqIterator iterates" <| fun _ ->
            let e = new SeqEnumerator(makeConsChain 3) :> IEnumerator
            testThreeElementSeqEnumerator e

        testCase "SeqIteratorReset at beginning" <| fun _ ->
            let e = new SeqEnumerator(makeConsChain 3) :> IEnumerator
            e.Reset()
            testThreeElementSeqEnumerator e

        testCase "SeqIteratorReset after move" <| fun _ ->
            let e = new SeqEnumerator(makeConsChain 3) :> IEnumerator
            e.MoveNext() |> ignore
            e.MoveNext() |> ignore
            e.Reset()
        ...
        testCase "SeqIteratorReset at end " <| fun _ ->
            let e = new SeqEnumerator(makeConsChain 3) :> IEnumerator
            e.MoveNext() |> ignore
            e.MoveNext() |> ignore
            e.MoveNext() |> ignore
...
[<Tests>]
let consISeqTests =
    testList "cons.seq tests" [
        
        testCase "cons.seq has correct values"   <| fun _ ->
            let c = makeConsChain 4
            let vals : obj list = [3;2;1;0]
            verifyISeqContents c vals

        testCase "cons on cons.seq has correct values"   <| fun _ ->
            let c = makeConsChain 4
            let d = (c:>ISeq).cons("a")
