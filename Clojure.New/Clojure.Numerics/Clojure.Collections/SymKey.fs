...
type Symbol private (meta: IPersistentMap, ns: string, name: string) = 
    ...
    override _.GetHashCode() = Util.hashCombine(name.GetHashCode(),Util.hash(ns))

    interface IHashEq with  
        member _.hasheq() =
            match hasheq with
            | None ->
                let h = Util.hashCombine(Murmur3.HashString(name),Util.hash(ns))
                hasheq <- Some h
                h
            | Some h -> h
...
type Keyword private (sym:Symbol) =
    ...
    static member intern(s) =
        let useSym s = if  (s:>IMeta).meta() |> isNull  then s else downcast (s:>IObj).withMeta(null) 
    ...
    interface IComparable<Keyword> with
        member _.CompareTo(k) = (sym:>IComparable<Symbol>).CompareTo(k.Symbol)
