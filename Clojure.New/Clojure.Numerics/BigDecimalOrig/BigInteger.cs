...
namespace clojure.lang
{
    ...
    public class BigInteger: IComparable, IConvertible, IEquatable<BigInteger>
    {
        ...
        private static readonly BigInteger _zero = new (0, Array.Empty<uint>());
        private static readonly BigInteger _one = new (1, new uint[] { 1 });
        private static readonly BigInteger _two = new (1, new uint[] { 2 });
        private static readonly BigInteger _five = new (1, new uint[] { 5 });
        private static readonly BigInteger _ten = new (1, new uint[] { 10 });
        private static readonly BigInteger _negativeOne = new (-1, new uint[] { 1 });
        ...
        public static bool TryParse(string s, int radix, out BigInteger v)
        {
            ...
            if (!TryParseUInt(s, index, firstGroupLen, radix, out data[^1]))
            {
                v = null;
                return false;
            }
        ...
        public string ToString(uint radix)
        {
            ...
            StringBuilder sb = new(rems.Count * RadixDigitsPerDigit[radix] + 1);
            ...
            AppendDigit(sb, rems[^1], radix, charBuf, false);
        ...
        public BigInteger Add(BigInteger y)
        {
            ...
            else
            {
                ...
                return c switch
                {
                    -1 => new BigInteger(-this._sign, Subtract(y._data, this._data)),
                    0 => new BigInteger(BigInteger.Zero),
                    1 => new BigInteger(this._sign, Subtract(this._data, y._data)),
                    _ => throw new InvalidOperationException("Bogus result from Compare"),
                };
            }
        }
        ...
        private static void DivMod(uint[] x, uint[] y, out uint[] q, out uint[] r)
        {
            ...
            int shift = (int)LeadingZeroCount(y[0]);
