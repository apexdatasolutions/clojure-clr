...
let IPersistentListTests = 
    testList "PersistenList Tests" [

        testCase "ctor for one element" <| fun _ ->
        ...
        testCase "create for several elements" <| fun _ ->
        ...
        testCase "Peek yields first element" <|  fun _ ->
        ...
        testCase "Pop loses first element"  <| fun _ ->
            let vs : obj list =  ["A";2;"B"]
            let xs : ResizeArray<obj> = ResizeArray(vs)  // We need an IList
            let p = PersistentList.create(xs)
            let p2 = p.pop()
        ...
        testCase "Pop on singleton yields empty" <| fun _ ->
            let p = PersistentList("abc") :> IPersistentStack
            let s = p.pop()
            Expect.equal (s.count()) 0 "Should be empty list"

        testCase "Double pop on singleton throws" <| fun _ ->
            let p = PersistentList("abc") :> IPersistentStack
            let f () = p.pop().pop() |> ignore
            Expect.throwsT<InvalidOperationException> f "should throw when pop empty"

        testCase "Empty has no elements" <| fun _ ->
            let p = PersistentList("abc") :> IPersistentCollection
            let e = p.empty()
            Expect.equal (e.count()) 0 "Should have no elements"

        testCase "Empty preserves meta" <| fun _ ->
            let p = PersistentList("abc") :> IObj
            let pm = p.withMeta(metaForSimpleTests) :?> IPersistentCollection
            let e = pm.empty() :?> IObj
        ...
        testCase "Verify some basic properties of PersistentList.ISeq" <| fun _ ->
            
            let p1 = PersistentList("abc")
            let p2 = (p1:>ISeq).cons("def")
            let p3 = p2.cons(7) :?> IPersistentList
            let vals : obj list = [7; "def"; "abc"]
            let p4 = (p3:?>IObj).withMeta(metaForSimpleTests) :?> ISeq
            ...
            let p5 = p4.cons("pqr")
            Expect.isTrue (Object.ReferenceEquals((p5:?>IMeta).meta(),(p4:?>IMeta).meta())) "Cons should preserve meta"
